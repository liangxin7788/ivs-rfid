package com.fun.business.sharon.utils.poi;
 ...
 * @Package: com.fun.business.sharon.utils.poi
 * @ClassName: PdfTwo
 * @Description: java类作用描述 参考 https://www.cnblogs.com/qlqwjy/p/8213989.html
 * @Author: liangxin
 * @CreateDate: 2020/4/8 16:35
 * @UpdateDate: 2020/4/8 16:35
 */
public class PdfTwo {
...
//关闭文档
    ...
    public void writePdf(String title, String cont, String createTime, String authorName) throws Exception {
        ...
        //2.建立一个书写器(Writer)与document对象关联，通过书写器(Writer)可以将文档写入到磁盘中。
        //创建 PdfWriter 对象 第一个参数是对文档对象的引用，第二个参数是文件的实际名称，在该名称中还会给出其输出路径。
        PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("C:\\ITextTest-image.pdf"));
        //3.打开文档
        document.open();
        //4.向文档中添加内容
        //通过 com.lowagie.text.Paragraph 来添加文本。可以用文本及其默认的字体、颜色、大小等等设置来创建一个默认段落
        Paragraph pt = new Paragraph("梁新:-" + title, keyfont);//设置字体样式
        ...
        document.add(new Paragraph("\n"));
        // 加入图片
        Image jpg = Image.getInstance("http://liangxin.fun/7ff4198723d145619c0ed1f04812a36f.jpg");
        jpg.setAlignment(0);
        document.add(jpg);
